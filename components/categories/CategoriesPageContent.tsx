...
export default function CategoriesPageContent(props: { noLayout?: boolean } = {}) {
  ...
  const content = (
    <>
                  ...
                  >
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        {!category.active && (
                          <span className="text-xs text-gray-500 bg-gray-200 px-2 py-1 rounded">غیرفعال</span>
                        )}
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="افزودن دسته‌بندی جدید" size="md">
        <CategoryForm
          formData={formData}
      ...
      >
        <CategoryForm
          ...
          onCancel={() => { setShowEditModal(false); setEditingCategory(null); resetForm(); }}
        />
      </Modal>
...
function CategoryForm({
  ...
  formData: { name: string; slug: string; description: string; order: number; active: boolean };
  setFormData: React.Dispatch<React.SetStateAction<{ name: string; slug: string; description: string; order: number; active: boolean }>>;
  isSaving: boolean;
  onSave: () => void;
  onCancel: () => void;
}) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="w-4 h-4"
        />
        <label htmlFor="active" className="text-sm font-medium">فعال</label>
      </div>
      <div className="flex gap-2 justify-end">
        <Button variant="outline" onClick={onCancel} disabled={isSaving}>انصراف</Button>
